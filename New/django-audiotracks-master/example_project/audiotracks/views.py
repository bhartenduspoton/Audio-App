...
def user_index(request, username, page_number=None):
    ...
    if request.method == 'GET' and 'search_text' in request.GET:
        ...
        if search_text=="" or search_text==None:
            ...
            user = get_object_or_404(User, username=username)
            tracks = user.tracks.order_by('-created_at').filter(Q(title__search=search_text) | Q(artist__search=search_text))
            page, tracks = paginate(tracks, page_number)
            base_path = urlresolvers.reverse('user_index', args=[username])
            print base_path
            return render_to_response("audiotracks/user_index.html", {
...
def search_index(request, username, page_number=None):
